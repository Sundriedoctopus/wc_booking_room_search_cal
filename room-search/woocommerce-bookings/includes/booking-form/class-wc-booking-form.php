...
class WC_Booking_Form {
	define ('WC_BOOKINGS_PLUGIN_URL' , '/wp-content/plugins/woocommerce-bookings/');
	/**
	...
	private function persons_field() {
		...
		if ( $this->product->has_persons() ) {
			...
			foreach ($term as $t) {
				if ( strpos($t->slug, 'entire-room') !== false ) {
					$has_entire = true;
				}
			}
			...
			if ($has_entire == true) {
				$this->add_field( array(
					...
					'max'   => $max_persons,
					'name'  => 'ref_persons',
					'label' => __( 'Number of Guests', 'woocommerce-bookings' ),
				) );
			}
			/* MOD SECTION END ============================================================== */
...
// JM EDIT START
		...
		// Get Bed Type
		if ( ! empty( $posted['wc_bookings_field_bed_type'] ) ) {
			$data['_bed_type'] = $_POST['wc_bookings_field_bed_type'];
		}
		// Get Original Guest QTY
		if ( ! empty( $posted['wc_bookings_field_ref_persons'] ) ) {
			$data['_ref_persons'] = $_POST['wc_bookings_field_ref_persons'];
		}

<!-- // Returning Guest Discount Start -->
		// Get Original Guest QTY
		if ( ! empty( $posted['wc_bookings_field_jo_discount'] ) ) {
			$data['_jo_discount'] = $_POST['wc_bookings_field_jo_discount'];
		}
		// Get Original Guest QTY
		if ( ! empty( $posted['wc_bookings_field_jm_discount'] ) ) {
			$data['_jm_discount'] = $_POST['wc_bookings_field_jm_discount'];
		}
		// Get Original Guest QTY
		if ( ! empty( $posted['wc_bookings_field_num_discount'] ) ) {
			$data['_num_discount'] = $_POST['wc_bookings_field_num_discount'];
		}
...
// JM EDIT END
		...
		if ( $this->product->has_persons() ) {
			...
			if ( isset( $posted['wc_bookings_field_ref_persons'] ) ) {
				$data[ __( 'Ref_Persons', 'woocommerce-bookings' ) ] = absint( $posted['wc_bookings_field_ref_persons'] );
				$data['_ref_persons'][0]                             = absint( $posted['wc_bookings_field_ref_persons'] );
			}
			...
			if ( $this->product->get_has_person_qty_multiplier() ) {
				if ( ! isset( $posted['wc_bookings_field_ref_persons'] ) ) {
					...
					$data['_qty'] = array_sum( $data['_ref_persons'] );
				}
			}
		}
	...
	public function is_bookable( $data ) {
		...
		if ( $this->product->has_persons() ) {
			...
			$ref_persons = array_sum( $data['_ref_persons'] );
			...
			if ( $this->product->get_max_persons() && $ref_persons > $this->product->get_max_persons() && $ref_persons != null ) {
				/* translators: 1: maximum persons */
				return new WP_Error( 'Error', sprintf( __( 'The maximum persons per group is %d', 'woocommerce-bookings' ), $this->product->get_max_persons() ) );
			}
			if ( $ref_persons != null && $ref_persons < 1 ) {
				/* translators: 1: minimum persons */
				return new WP_Error( 'Error', sprintf( __( 'The minimum persons per group is %d', 'woocommerce-bookings' ), $this->product->get_min_persons() ) );
			}
